public class LeasingCost {
    ...
    */
      public static Vehicle createVehicle(String description){
       //my code for the leasing cost vehicles.
       ...
       int begin = description.indexOf("name");
           int end = -1;
           String newString = description.substring(begin);
  
           end = newString.indexOf(";");
           String name = newString.substring(5, end);
  
           begin = description.indexOf("type");
           newString = description.substring(begin);
           end = newString.indexOf(";");
           String type = newString.substring(5, end);
  
           begin = description.indexOf("mile/unit");
           newString = description.substring(begin);
           end = newString.indexOf(";");
           String usage = newString.substring(10, end);
           ...
           if (type.equalsIgnoreCase("gas")) {
               ...
               begin = description.indexOf("charger");
               newString = description.substring(begin);
               end = newString.indexOf(";");
               String charger = newString.substring(8, end);
               fuel = new Fuel(Double.valueOf(usage), Double.valueOf(charger));
           }
  
           begin = description.indexOf("due");
           newString = description.substring(begin);
           end = newString.indexOf(";");
           String due = newString.substring(4, end);
  
           begin = description.indexOf("length");
           newString = description.substring(begin);
           end = newString.indexOf(";");
           String length = newString.substring(7, end);
  
           begin = description.indexOf("monthly");
           newString = description.substring(begin);
           end = newString.indexOf(";");
           String monthly = newString.substring(8, end);
  
           begin = description.indexOf("allowance");
           newString = description.substring(begin);
           end = newString.indexOf(";");
           String allowance = newString.substring(10, end);
  
           Lease lease = new Lease(Double.valueOf(due), Integer.valueOf(length), Double.valueOf(monthly),
    ...
    */
      public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
             ...
             for (int i = 0; i < vehicles.length; i++) {
                 typeoffuel = vehicles[i].getFuel().getType();
    
                 if (typeoffuel == type){
                     fuelcosts = computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(),vehicles[i].getLease().getMileageAllowance(),gasPrice);
                     vehicles[i].setFuelCost(fuelcosts);
    
                     leasecosts = computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(),vehicles[i].getLease().getMonthlyCost());
                     total = fuelcosts + leasecosts;
                     ...
                     fuelcosts = computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(),electricityPrice);
                     vehicles[i].setFuelCost(fuelcosts);
    
                     leasecosts = computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost());
                     total = fuelcosts + leasecosts + vehicles[i].getFuel().getCharger();
